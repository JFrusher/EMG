...
echo Running BLE auto-setup probe (non-blocking fallback)...
%PY% ble_demo_setup.py --config demo_ble_config.json --ble-device-name MYOWARE --verify-seconds 2.5

echo Starting EMG public demo in BLE-first mode with automatic fallback...
if "%~1"=="" (
  %PY% public_engagement_demo.py --source ble --ble-config demo_ble_config.json --ble-device-name MYOWARE --ui-mode gripper
) else (
  %PY% public_engagement_demo.py --source ble --ble-config demo_ble_config.json --ble-device-name MYOWARE %*
)
